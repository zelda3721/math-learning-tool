...
写任何 beat 之前，先在内部回答三个问题（答案落进 essence_rationale 与 beat 设计）：

1. **答案为什么成立？** 找出使结论不可避免的那条决定性关系——某个守恒量、相等关系、
   单调趋势或一一对应。这是本次视频要"演给学生看"的东西，不是解题步骤的复述。
2. **这条关系如何变成可见的对象与变化？** 决定性关系必须由图形对象的状态变化承载：
   守恒→总量在重组前后可见地不变；差额→缺口被逐步补齐；相等→两侧同步变化保持平衡。
3. **学生盯着哪一个变化就能自己推出结论？** 那个变化就是全片的注意力主线；
   其他一切都为它让路。

**推理原型（按论证语义选择，不是题型库）**——选一条最能让结论"不可避免"的：
- **守恒重组**：总量不变，只改变分组/形态，重组后关系显形；
- **差额收拢**：假设一个基线，让实际与假设的差距被逐步、等量地补齐，补几次就是答案；
- **等量同变**：相等关系两侧执行同一操作，平衡状态全程可见（天平）；
- **单调逼近**：一列可见状态单调地逼近一个固定参照，逼近本身就是结论；
- **一一对应**：两组成员逐对配对，剩余部分即为差异；
- **分解重组**：把整体拆成可数/可测的部分，再拼回去核对总量。

essence_rationale 必须**指名这条决定性关系**（"因为总量守恒，所以…"、"每换一个补 2，
差 24 就要换 12 个"），不能只描述画面里发生了什么动画。
