...
_GRADE_GUIDANCE: dict[str, str] = {
    "elementary_lower": (
        "小学低年级（1-3）：默认数形结合——把题目里的数量、关系、变化全部画出来"
        "（实物 / 圆点 / 方块 / 线段 / 流程箭头），用动画一步步演示。"
        "禁止使用方程 / 未知数 / 代数式（题面没写方程就不能引入未知数）。"
        "推荐画图法、实物演示、凑十法、逐步数数。语言要简单具象。"
    ),
    "elementary_upper": (
        "小学高年级（4-6）：默认数形结合——优先假设法、列表法、画图分析、逆推法、"
        "线段图，把关系画出来再算。鸡兔同笼必须用假设法。"
        "**禁用方程**——只有当题面明确给出方程、或明确要求用方程/设未知数解时，"
        "才允许列方程；否则一律用图形 + 动画把数量关系演示出来。"
    ),
    "middle": (
