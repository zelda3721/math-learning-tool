# Math Tutor - AI数学辅导工具

基于大模型多Agent技术和Manim库的数学辅导工具，支持从小学到大学微积分的多年级数学题目，生成直观的数形结合可视化视频。

## ✨ 主要特点

- 🧠 **智能分析**：自动识别题目难度，智能分流处理
- 📝 **详细解答**：生成分步骤解题过程，验证答案正确性
- 🎬 **可视化演示**：Manim动画展示解题过程
- 🎯 **多年级支持**：小学低年级 → 小学高年级 → 初中 → 高中 → 大学微积分
- 🔄 **LangGraph工作流**：图状态机架构，支持条件分支和错误恢复

## 🏗️ 系统架构

```
┌─────────────────────────────────────────────────────────────┐
│                    React + TypeScript 前端                   │
│              (Vite + TailwindCSS + TanStack Query)          │
└──────────────────────────┬──────────────────────────────────┘
                           │ REST API
┌──────────────────────────▼──────────────────────────────────┐
│                    FastAPI 后端 (Python)                     │
│  ┌────────────────────────────────────────────────────────┐ │
│  │                   LangGraph 工作流                      │ │
│  │  ┌──────────┐    ┌──────────┐    ┌──────────────────┐  │ │
│  │  │ 分类器   │───►│ 深度分析 │───►│ 解题+验证        │  │ │
│  │  └────┬─────┘    └──────────┘    └────────┬─────────┘  │ │
│  │       │ (简单题)                          │            │ │
│  │       └─────────────────────────┐         ▼            │ │
│  │                                 ▼    ┌──────────┐      │ │
│  │                            ┌────────►│ 可视化   │      │ │
│  │                            │        └────┬─────┘      │ │
│  │                            │             ▼            │ │
│  │  ┌──────────┐    ┌─────────┴──┐    ┌──────────┐      │ │
│  │  │ 降级处理 │◄───│ Manim执行  │◄───│ 调试修复 │◄─┐   │ │
│  │  └──────────┘    └────────────┘    └──────────┘  │   │ │
│  │                                          │        │   │ │
│  │                                          └────────┘   │ │
│  └────────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────────┘
```

### 技术栈

| 层级 | 技术 |
|------|------|
| 前端 | React 18, TypeScript, Vite, TailwindCSS |
| 后端 | FastAPI, Python 3.12, Pydantic |
| Agent | LangGraph, LangChain |
| 可视化 | Manim CE |
| 部署 | Docker, docker-compose |

## 📁 项目结构

```
math-learning-tool/
├── backend/                      # Python 后端
│   ├── src/math_tutor/
│   │   ├── api/                  # FastAPI 路由
│   │   ├── application/          # 用例层
│   │   ├── domain/               # 领域层
│   │   ├── infrastructure/       # 基础设施层
│   │   │   ├── agents/
│   │   │   │   ├── nodes/        # LangGraph 节点
│   │   │   │   └── langgraph_engine.py
│   │   │   ├── skills/           # 技能系统
│   │   │   ├── manim/            # Manim 执行器
│   │   │   └── llm/              # LLM 服务
│   │   └── config/               # 配置
│   ├── tests/                    # 测试
│   └── pyproject.toml
├── frontend/                     # React 前端
│   ├── src/
│   │   ├── components/           # React 组件
│   │   └── services/             # API 服务
│   └── package.json
├── docker-compose.yml
└── .env.example
```

## 🚀 快速开始

### 前提条件

- Python 3.12+
- Node.js 20+
- FFmpeg (Manim依赖)
- LLM API密钥 (DeepSeek/OpenAI/本地模型)

### 方式一：Docker部署（推荐）

```bash
# 1. 克隆项目
git clone https://github.com/your-repo/math-learning-tool.git
cd math-learning-tool

# 2. 配置环境变量
cp .env.example .env
# 编辑 .env 设置 LLM_API_KEY

# 3. 启动服务
docker-compose up -d

# 访问：
# - 前端: http://localhost:3000
# - API: http://localhost:8000/api/docs
```

### 方式二：本地开发

#### 后端

```bash
cd backend

# 创建虚拟环境
python -m venv .venv
source .venv/bin/activate  # Windows: .venv\Scripts\activate

# 安装依赖
pip install -e ".[dev]"

# 启动开发服务器
uvicorn math_tutor.api.main:app --reload --host 0.0.0.0 --port 8000

# API文档: http://localhost:8000/api/docs
```

#### 前端

```bash
cd frontend

# 安装依赖
npm install

# 启动开发服务器
npm run dev

# 访问: http://localhost:5173
```

## ⚙️ 配置说明

### 环境变量 (.env)

```bash
# LLM配置
LLM_API_BASE=https://api.deepseek.com/v1
LLM_API_KEY=your_api_key
LLM_MODEL=deepseek-chat

# Manim配置
MANIM_QUALITY=low  # low/medium/high

# 可选：使用本地模型
# LLM_API_BASE=http://localhost:9997/v1
# LLM_MODEL=qwen2.5-72b-instruct
```

### 年级配置

系统支持5个教育阶段，每个阶段有不同的思维风格和可视化风格：

| 年级 | 思维风格 | 可视化特点 |
|------|----------|------------|
| 小学低年级 | 具象思维 | 彩色图标、大字体、慢动画 |
| 小学高年级 | 半抽象 | 图形化、步骤清晰 |
| 初中 | 抽象思维 | 几何图形、公式推导 |
| 高中 | 形式化 | 函数图像、数学符号 |
| 大学 | 高度抽象 | 3Blue1Brown风格 |

## 🧪 运行测试

```bash
# 后端测试
cd backend
pytest tests/ -v

# 前端测试
cd frontend
npm run test
```

## 📝 API端点

| 方法 | 路径 | 说明 |
|------|------|------|
| `GET` | `/api/health` | 健康检查 |
| `GET` | `/api/v1/grades` | 获取年级列表 |
| `POST` | `/api/v1/problems/process` | 完整处理流程 |
| `POST` | `/api/v1/problems/analyze` | 仅分析题目 |
| `POST` | `/api/v1/problems/solve` | 仅解题 |

### 请求示例

```bash
curl -X POST http://localhost:8000/api/v1/problems/process \
  -H "Content-Type: application/json" \
  -d '{
    "problem": "小明有25个糖果，给了小红8个，现在有多少个？",
    "grade": "elementary_upper"
  }'
```

## 🔧 调试指南

### 常见问题

1. **Manim执行失败**
   - 确保安装了FFmpeg: `ffmpeg -version`
   - 检查media目录权限

2. **LLM连接失败**
   - 检查.env中的API配置
   - 验证API密钥有效性

3. **前端无法连接后端**
   - 确保后端运行在8000端口
   - 检查CORS配置

### 日志查看

```bash
# 后端日志
docker-compose logs backend -f

# 或直接运行时
uvicorn math_tutor.api.main:app --reload --log-level debug
```

## 📄 许可证

MIT License

## 🙏 致谢

- [LangGraph](https://github.com/langchain-ai/langgraph) - Agent工作流
- [Manim CE](https://github.com/ManimCommunity/manim) - 数学动画库
- [DeepSeek](https://www.deepseek.com) - 大语言模型
- [FastAPI](https://fastapi.tiangolo.com) - 后端框架
- [React](https://react.dev) - 前端框架
