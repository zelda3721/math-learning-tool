...
/**
 * 这一条像是「只有题号、没有题干」——**下一页那道题的头**。
 *
 * 它只用来决定一件事：**下一页开头那段算不算新题**（而不是并进上一道题）。
 * **不用来决定要不要丢掉这一条**——那两件事一绑，就变成了一个两头都会出错的
 * 单点判断：判松了丢真题（页底一道开头很短的题被整条丢掉），
 * 判紧了吞新题（跨页那道被并进上一题）。实机上两种都发生过，一次丢两三道。
 *
 * 拆开之后这个判断可以宽松：认错了只是让下一页开头多当一次新题，
 * 而那一条本身照样会被抽取，抽出来是题就留下、是题号就滤掉。
 *
...
export function isDanglingLabel(item: LayoutItem): boolean {
   ...
   *
   * 判据：**在页面最下方**，而且**开头这几个字不像题干**。
   * 试过只看「preview 与 label 相同」（漏了练习7——模型给的 preview 是
   * 旁边的章节标题）、只看框高（不稳，同一页跑两次框时窄时宽），
   * 最后是这两条合起来。
   */
   ...
   */
  // 判松一点：漏认的代价（跨页那道被并掉）比误认（多当一次新题）大得多
  return (
    preview === "" ||
    preview === label ||
    /^[一二三四五六七八九十]+\s*[、.．]/.test(preview) ||
    preview.length < 12
  );
}
